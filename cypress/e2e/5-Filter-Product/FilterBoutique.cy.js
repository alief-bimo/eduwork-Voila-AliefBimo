...
describe('Automate Filter based on Boutique Availability Viola', () => {
    ...
    it('Should filter the search result based on item available on Jakarta - Mall of Indonesia Boutique', () => {
        ...
        cy.wait(6000)
        cy.get('[data-test-id="CT-filter-checkbox"]').eq(4).click   ()
        cy.get('#search-result-product-list-44442 > ._1nvnchg4').click()
        cy.wait(7000)
    ...
    it('Should filter the search result based on item available on Jakarta - Pantai Indah Kapuk Boutique', () => {
        cy.searchItem('nike{enter}')
        cy.wait(6000)
        cy.get('[data-test-id="CT-filter-checkbox"]').eq(5).click({waitForAnimations: false})
        cy.get('p').contains('1 Low SE Phantom Repaired Denim Swoosh Men').click()
        cy.wait(7000)
    ...
    it('Should filter the search result based on item available on Jakarta - Pondok Indah Boutique', () => {
        cy.searchItem('nike{enter}')
        cy.wait(6000)
        cy.get('[data-test-id="CT-filter-checkbox"]').eq(6).click({waitForAnimations: false})
        cy.get('#search-result-product-list-43056 > ._1nvnchg4').click()
        cy.wait(7000)
    ...
    it('Should filter the search result based on item available on Surabaya - Bukit Darmo Golf Boutique', () => {
        cy.searchItem('nike{enter}')
        cy.wait(6000)
        cy.get('[data-test-id="CT-filter-checkbox"]').eq(7).click({waitForAnimations: false})
        cy.get('#search-result-product-list-2760 > ._1nvnchg4').click()
        cy.wait(7000)
